...
export class GuestDocumentController {
  ...
  public static uploadDocument = [
    ...
    async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
      try {
        ...
        logger.info(`[GuestDocumentController] === DOCUMENT UPLOAD START for guest ${guestId} ===`);

        const currentUser = req.user!;
        
        if (!req.file) {
          logger.error('[GuestDocumentController] No document file provided');
          res.status(400).json({
            success: false,
            error: 'Bad Request',
        ...
        logger.info('[GuestDocumentController] File received:', {
        ...
        const key = s3Service.generateDocumentKey('guest', guestId, file.originalname);
        ...
        logger.info('[GuestDocumentController] S3 upload successful:', s3Result);
        ...
        if (!dbFile.success) {
          logger.error('[GuestDocumentController] Database save failed, cleaning up S3...');
        ...
        logger.info(`[GuestDocumentController] === DOCUMENT UPLOAD END for guest ${guestId} ===`);
        ...
        logger.error('[GuestDocumentController] CRITICAL ERROR during document upload:', {
  ...
  public static async getDocuments(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      logger.info(`[GuestDocumentController] Getting documents for guest ${guestId}`);

      // Get files from file_uploads table
      const result = await FileService.getEntityFiles(currentUser, 'GUEST', guestId);
      ...
      logger.info(`[GuestDocumentController] Returning ${documents.length} documents`);
      ...
      logger.error('[GuestDocumentController] Error getting documents:', error);
      res.status(500).json({
        success: false,
        error: 'Internal Server Error',
  ...
  public static async deleteDocument(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const currentUser = req.user!;
      const { guestId, documentId } = req.params;

      logger.info(`[GuestDocumentController] Deleting document ${documentId} for guest ${guestId}`);

      // Get file info first
      const fileResult = await FileService.getEntityFiles(currentUser, 'GUEST', guestId);
      ...
      if (s3Service.isConfigured()) {
        try {
          ...
          logger.warn(`[GuestDocumentController] Failed to delete from S3: ${error}`);
        }
      }
      ...
      logger.error('[GuestDocumentController] Error deleting document:', error);
      res.status(500).json({
        success: false,
        error: 'Internal Server Error',
  ...
  public static async getDownloadUrl(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      logger.info(`[GuestDocumentController] Getting download URL for document ${documentId}`);
      ...
      logger.error('[GuestDocumentController] Error getting download URL:', error);
      res.status(500).json({
        success: false,
        error: 'Internal Server Error',
