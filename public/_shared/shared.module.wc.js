...
import { LocaleHelper, Localizable, Base, GlobalEvents, AjaxHelper, AsyncHelper, BrowserHelper, DateHelper, DomHelper, StringHelper, DataGenerator, Fullscreen, Rectangle, VersionHelper, Events, Button, DemoCodeEditor, Toast, Tooltip, Widget, HintFlow } from '../../build/scheduler.wc.module.js';
/* global RC */
// Load product config
...
class Shared extends Localizable(Events()) {
            ...
            codepenButton                 = new Button({
                ...
                async onClick({ source }) {
                    ...
                    const
                        ...
                        themeName         = themeLink?.href.match(/[^/]+\.([^.]+)\.css$/)?.[1] ?? `stockholm`,
                        apiStyles         = [
                            ...document.head.querySelectorAll('link[rel="stylesheet"][data-bryntum-theme]')
                        ].map(link =>
                            `<link rel="stylesheet" href="${link.getAttribute('href').replace(/.*\/build\//, `${buildUrl}/`)}?${hash}" data-bryntum-theme>`
                        ).join('\n'),
                        demoStyles        = (me.appConfig.index?.stylesheets ?? [])
                            .map(({ href }) => `<link rel="stylesheet" href="${productUrl}/examples/${demoName.toLowerCase()}/${href}?${hash}">`),
                        formConfig        = {
                            tag      : 'form',
                            style    : 'display:none',
    ...
    getStoredThemeId(defaultThemeId) {
        ...
        const
            themeId    = queryString.theme || (!testMode && BrowserHelper.getLocalStorageItem('b-example-theme')) || defaultThemeId,
            themeNames = {
                stockholm       : 'stockholm',
                classic         : 'classic',
                default         : 'classic',
...
document.addEventListener('DOMContentLoaded', () => {
    ['grid', 'scheduler', 'schedulerPro', 'gantt', 'calendar', 'taskboard', 'histogram', 'treegrid'].forEach(productId => {
        ...
        document.getElementById('skip-to-content')?.addEventListener('click', () => {
            bryntum.query(bryntum.product.name).element.querySelector('[aria-label],[aria-labelledby],input,button,a')?.focus();
        });
    });
