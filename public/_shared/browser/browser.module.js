import { Store, AjaxHelper, BrowserHelper, DomSync, DomHelper, EventHelper, Toolbar, Popup, Widget, GlobalEvents, VersionHelper, FunctionHelper } from '../../../build/scheduler.module.js';
...
class ExamplesApp {
    constructor() {
        ...
        const
            version         = VersionHelper.getVersion('core'),
    ...
    filterDemos() {
        const
            me                      = this,
            { framework, newDemos } = me.toolbar.widgetMap,
            frameworks              = framework.widgetMap,
            selectedFramework       = Object.keys(frameworks).find(b => frameworks[b].pressed);

        this.exampleStore.clearFilters();
        this.exampleStore.filter(record => {
            const
                isFramework    = !selectedFramework || record.tab === selectedFramework,
                isNewOrUpdated = !newDemos.checked || this.compareVersion(record.updated) || this.compareVersion(record.since);
            return isFramework && isNewOrUpdated;
        });

        if (selectedFramework) {
            BrowserHelper.setLocalStorageItem('bryntum-framework', selectedFramework);
            me.jumpTo.items = [
                {
                    id   : 'top',
                    text : 'Top'
                ...
                me.exampleStore.groupRecords.map(r => ({
                    id   : r.id,
        ...
        me.jumpTo.hidden = selectedFramework && selectedFramework !== 'js';
    }
                        ...
                        },
                        {
                            ...
                            children : [
                                ...
                                example.version && {
                                    className : {
                                        version : 1,
                                        thin    : example.folder.endsWith('-thin')
                                    },
