...
const bryntumUrl = 'https://bryntum.com',
  ...
  noEditor = queryString.noEditor != null || window.matchMedia('(max-width: 700px)').matches;
if (!isDemoBrowser) {
...
class Shared extends Localizable(Events()) {
  ...
  constructor() {
    ...
    if (!noEditor && !isDemoBrowser && !testMode || me.preload) {
      setTimeout(() => {
        me.createEditor();
      }, me.preloadMs);
    }
  ...
  async loadAppConfig() {
    ...
    try {
      ...
      me.appConfig = (await AjaxHelper.get(appConfigUrl, {
        parseJson: true
      })).parsedJson;
    } catch (error) {
      me.appConfig = {
    ...
    if (me.container) {
      ...
      me.codepenButton.hidden = isDemoBrowser || me.appConfig.disableCodePen || !DemoCodeEditor.isModule && me.appConfig.merge;
    }
  }
  ...
  insertHeader() {
    ...
    const tools = document.getElementById('tools'),
      trialButton = isBryntumCom || testMode ? new Button({
        appendTo: tools,
        id: 'trial-button',
      ...
      codepenButton = new Button({
        ...
        async onClick({
          ...
          var _codeEditor, _codeEditor2, _pathName$match, _themeLink$href$match, _me$appConfig$index;
          const {
              element
            } = me,
            {
          ...
          const loadedCode = ((_codeEditor = codeEditor) === null || _codeEditor === undefined ? undefined : _codeEditor.codeCache['app.js']) || ((_codeEditor2 = codeEditor) === null || _codeEditor2 === undefined ? undefined : _codeEditor2.codeCache['app.module.js']),
            ...
            refineHTMLSnippet = html => {
              ...
              [...codePenHTML.querySelectorAll('script')].forEach(script => {
                ...
                if (src !== null && src !== undefined && src.includes('app.module.js') || src !== null && src !== undefined && src.includes('../_shared/locales/examples.locales.umd.js')) {
                  script.remove();
                }
              });
            ...
            demoName = (_pathName$match = pathName.match(/(?:examples\/)([^/?#]+)/)) === null || _pathName$match === undefined ? undefined : _pathName$match[1],
            importStatements = (() => {
              // Parse source code imports and generate import header for CodePen
              const thinImports = [...loadedCode.matchAll(/\/build\/thin\/(.*?)'/gm).map(m => m[1])],
              ...
              return ['// Import bundles and expose all Bryntum classes on window to simplify coding at CodePen', ...imports, assign, ''].join('\n');
            })(),
            demoCode = loadedCode.replace(/^\s*import\s+.*?;.*$/gm, '')
            // Replace relative links with absolute paths
            .replace(/(resourceImagePath\s*:\s*)['"][^'"]*['"](\s*,?)/, `$1'${productUrl}/examples/_shared/images/users/'$2`).replace(/\.\.\/_datasets\/([^/]+\.json)/g, `${productUrl}/examples/_datasets/$1`).replace(/(?:\.\.?\/)*data\/[^/]+\.json/g, `${productUrl}/examples/${demoName.toLowerCase()}/$&`)
            // Fix CORS issue for remote data loading
            .replace(/(\s*)readUrl\s*:/g, `$1fetchOptions : { credentials : 'omit' },  // required to bypass CORS at CodePen$1readUrl :`).replace(/^(\s*)loadurl\s*:\s*(['"].+?['"])/gim, `$1transport: {\n$1\tload: {\n$1\t\turl: $2\n$1\t}\n$1}`).replace(/(^\s*)(url\s*:\s*['"][^'"]*['"])(,?)/gim, `$1$2,\n$1credentials : 'omit'$3 // required to bypass CORS at CodePen`).trimStart(),
            themeLink = document.querySelector('[rel="stylesheet"][data-bryntum-theme]'),
            themeName = (themeLink === null || themeLink === undefined || (_themeLink$href$match = themeLink.href.match(/[^/]+\.([^.]+)\.css$/)) === null || _themeLink$href$match === undefined ? undefined : _themeLink$href$match[1]) ?? `stockholm`,
            apiStyles = [...document.head.querySelectorAll('link[rel="stylesheet"][data-bryntum-theme]')].map(link => `<link rel="stylesheet" href="${link.getAttribute('href').replace(/.*\/build\//, `${buildUrl}/`)}?${hash}" data-bryntum-theme>`).join('\n'),
            demoStyles = (((_me$appConfig$index = me.appConfig.index) === null || _me$appConfig$index === undefined ? undefined : _me$appConfig$index.stylesheets) ?? []).map(({
              href
            }) => `<link rel="stylesheet" href="${productUrl}/examples/${demoName.toLowerCase()}/${href}?${hash}">`),
            formConfig = {
              tag: 'form',
              style: 'display:none',
              ...
              children: [{
                ...
                value: StringHelper.safeJsonStringify({
                  ...
                  html: ['<html lang="en">', '<head>', apiStyles, `<link rel="stylesheet" href="${productUrl}/examples/_shared/shared.css?${hash}">`, demoStyles, '</head>', '<body>', htmlBody, me.localeManager.locale.localeName !== 'En' ? `<script src="${productUrl}/examples/_shared/locales/examples.locale.${me.localeManager.locale.localeName}.umd.js?${hash}"></script>` : '', '</body>', '</html>'].join('\n'),
                  js: [importStatements, '// Apply current theme', `document.body.classList.add("b-theme-${themeName}");\n`, demoCode, me.localeManager.locale.localeName !== 'En' ? `LocaleManager.applyLocale('${me.localeManager.locale.localeName}'); // Apply localization` : ''].join('\n')
                })
              }]
            },
      ...
      codeButton = noEditor ? null : new Button({
        ref: 'codeButton',
        icon: 'b-icon-code',
  ...
  loadDescription() {
    ...
    ...(!themes[theme] || body.matches('.b-theme-custom') ? [] : [themeCombo = {
      ...
      primaryFilter(record) {
        ...
        return record.text.toLowerCase().startsWith(value) || record.value.toLowerCase().startsWith(value);
      }
    }];
  ...
  getStoredThemeId(defaultThemeId) {
    ...
    const themeId = queryString.theme || !testMode && BrowserHelper.getLocalStorageItem('b-example-theme') || defaultThemeId,
      themeNames = {
        stockholm: 'stockholm',
        classic: 'classic',
        default: 'classic',
  ...
  fireMouseEvent(type, target, offset = [0, 0], showCursor) {
    ...
    if (showCursor) {
      if (!this.cursorElement) {
        ...
        this.cursorElement.className = 'b-fa b-fa-mouse-pointer';
      ...
      var _this$cursorElement;
      (_this$cursorElement = this.cursorElement) === null || _this$cursorElement === undefined || _this$cursorElement.remove();
    }
  ...
  async createEditor() {
    ...
    if (!codeEditor) {
      ...
      codeEditor.editor.addCommand(monacoInstance.KeyMod.CtrlCmd | monacoInstance.KeyCode.KeyS,
      // Ctrl/Cmd + S
      function () {
...
document.addEventListener('DOMContentLoaded', () => {
  ['grid', 'scheduler', 'schedulerPro', 'gantt', 'calendar', 'taskboard', 'histogram', 'treegrid'].forEach(productId => {
    var _window$productId, _document$getElementB;
    ...
    (_document$getElementB = document.getElementById('skip-to-content')) === null || _document$getElementB === undefined || _document$getElementB.addEventListener('click', () => {
      var _bryntum$query$elemen;
      (_bryntum$query$elemen = bryntum.query(bryntum.product.name).element.querySelector('[aria-label],[aria-labelledby],input,button,a')) === null || _bryntum$query$elemen === undefined || _bryntum$query$elemen.focus();
    });
  });
