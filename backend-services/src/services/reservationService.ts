...
export class ReservationService {
  ...
  async getReservations(filters: ReservationFilters) {
    try {
      ...
      loggerService.info('Fetching reservations with filters', { where, skip, limit });
      ...
      loggerService.error('Error fetching reservations', error);
  ...
  async getReservationById(id: string, userId?: string) {
    try {
      ...
      loggerService.error('Error fetching reservation by ID', error);
      throw error;
    }
  }
  ...
  async createReservation(data: CreateReservationRequest, userId: string) {
    try {
      ...
      loggerService.info('Reservation created successfully', { 
      ...
      loggerService.error('Error creating reservation', error);
      throw error;
    }
  }
   ...
   * Update reservation
   */
  async updateReservation(id: string, data: UpdateReservationRequest, userId: string) {
    try {
      ...
      // Calculate new total amount if dates or property changed
      ...
      loggerService.info('Reservation updated successfully', { 
        reservationId: id, 
        userId 
      });
      ...
      loggerService.error('Error updating reservation', error);
      throw error;
    }
  }
  ...
  async deleteReservation(id: string, userId: string) {
    try {
      ...
      loggerService.info('Reservation deleted successfully', { 
        reservationId: id, 
        userId 
      });
    } catch (error) {
      loggerService.error('Error deleting reservation', error);
      throw error;
    }
  }
   ...
   */
  async updateReservationStatus(id: string, data: ReservationStatusUpdate, userId: string) {
    try {
      const reservation = await prisma.reservation.findUnique({
        where: { id }
      });
      ...
      loggerService.info('Reservation status updated successfully', { 
        reservationId: id, 
        userId,
      ...
      loggerService.error('Error updating reservation status', error);
      throw error;
    }
  }
   ...
   * Check-in guest
   */
  async checkInGuest(id: string, userId: string) {
    try {
      const reservation = await prisma.reservation.findUnique({
        where: { id }
      });
      ...
      loggerService.info('Guest checked in successfully', { 
        reservationId: id, 
        userId 
      });
   ...
   * Check-out guest
   */
  async checkOutGuest(id: string, userId: string) {
    try {
      const reservation = await prisma.reservation.findUnique({
        where: { id }
      });
      ...
      loggerService.info('Guest checked out successfully', { 
        reservationId: id, 
        userId 
      });
      ...
      loggerService.error('Error checking out guest', error);
      throw error;
    }
  }
   ...
   * Mark as no-show
   */
  async markAsNoShow(id: string, userId: string) {
    try {
      const reservation = await prisma.reservation.findUnique({
        where: { id }
      });
      ...
      loggerService.info('Reservation marked as no-show', { 
        reservationId: id, 
        userId 
      });
      ...
      loggerService.error('Error marking reservation as no-show', error);
      throw error;
    }
  }
   ...
   */
  async getReservationCalendar(propertyId?: string, startDate?: string, endDate?: string, userId?: string) {
    try {
      const where: any = {};
      ...
      const calendar: CalendarEvent[] = reservations.map(reservation => ({
        id: reservation.id,
      ...
      loggerService.error('Error fetching reservation calendar', error);
      throw error;
    }
  }
   ...
   */
  async getReservationStats(propertyId?: string, startDate?: string, endDate?: string, userId?: string) {
    try {
      const where: any = {};
      ...
      loggerService.error('Error fetching reservation statistics', error);
      throw error;
    }
  }
  ...
  async getReservationSources() {
    try {
      ...
      loggerService.error('Error fetching reservation sources', error);
      throw error;
    }
  }
   ...
   */
  async getAvailableProperties(startDate?: string, endDate?: string, guests?: number, userId?: string) {
    try {
      const where: any = {
        isActive: true,
        isPublished: true
      ...
      // Filter out properties that are not available for the given dates
      if (startDate && endDate) {
      ...
      loggerService.error('Error fetching available properties', error);
      throw error;
    }
  }
   ...
   */
  private async updateAvailability(propertyId: string, startDate: string, endDate: string, status: string) {
    try {
      ...
      loggerService.error('Error updating availability', error);
      throw error;
    }
  }

  /**
   * Map database reservation to response format
   */
  private mapReservationToResponse(reservation: any): Reservation {
    return {
      id: reservation.id,
      propertyId: reservation.propertyId,
      propertyName: reservation.property?.name || '',
