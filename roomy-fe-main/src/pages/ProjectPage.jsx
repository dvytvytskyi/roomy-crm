...
const ProjectPage = () => {
    ...
    const [guestAmount, setGuestAmount] = useState(location.state?.data?.guest || 1)
    ...
    // Handle location.state with fallback
    const initialDates = location.state?.data?.value 
        ? location.state.data.value.map(obj => dayjs(obj.$d))
        : [defaultCheckIn, defaultCheckOut];
    
    let dataValueDayjs = initialDates;
    ...
    const [data, setData] =  useState({
        ...
        "guest": location.state?.data?.guest || 1,
        "pets": null,
        "neigh": null
    })
    ...
    useEffect(() => {
        const fetchPropertyDetails = async () => {
            try {
                ...
                // If we have project from location.state, use it immediately
                ...
                const response = await axios.get(API_ENDPOINTS.PROPERTIES.DETAILS(propertyId));
    ...
    useEffect(() => {
        if (data.value !== null && data.guest !== null) {
    ...
    function calculateTotalPrice() {
        let totalPrice = 0;
        avaibleProjectDatas && avaibleProjectDatas?.data?.days.forEach(priceData => {
            totalPrice += parseInt(priceData.price);
        });
        return totalPrice
    }
    const totalPrice = calculateTotalPrice()
    console.log(avaibleProjectDatas)
    ...
    return (<div className="project-page">
        ...
        <div className="gallery">
            <div className="photos">
                ...
                <div className="more-photos" onClick={() => setShowModal(true)}></div>
            </div>
        </div>
            ...
            <div className="info">
                <div className="name-description">
                    <div className="name-guests">
                        ...
                        <div className="guests">4 guests | {project.beds === 0 ? 'Studio' : project.beds + ' bedrooms'} | {project.bathrooms} bathrooms</div>
                    </div>
                ...
                <div className="select-move-in">
                    <div className="info-title">Select move-in dates</div>
                </div>
            </div>

            <div className="select-dates">
                {
                    datesSelected ? <div className="checkout-form">
                            <div className="title">{project.prices.currency} {project.prices.basePrice} <span>night</span></div>

                            <div className="check-in-out">
                                <div className="check">
                                    <div>{serachDay.$y}/{serachDay.$M < 10 ? '0' + serachDay.$M : serachDay.$M}/{serachDay.$D < 10 ? '0' + serachDay.$D : serachDay.$D}</div>
                                    <div>Check-in</div>
                                </div>
                                <div className="check">
                                    <div>{serachFinalDay.$y}/{serachFinalDay.$M < 10 ? '0' + serachFinalDay.$M : serachFinalDay.$M}/{serachFinalDay.$D < 10 ? '0' + serachFinalDay.$D : serachFinalDay.$D}</div>
                                    <div>Check-out</div>
                                </div>
                            </div>
                                    ...
                                    <div className="dog-child">
                                        ...
                                        <div className={`child ${includeChild && 'active'}`}>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25"
                                                 viewBox="0 0 25 25"
                                                 fill="none">
                                                <g clip-path="url(#clip0_972_4655)">
                                                    <path
                            ...
                            <div className="charged">You won’t be charged now</div>

                            <div className="price">
                                <div className="nights">
                                    <div> د.إ {project.prices.basePrice}</div>
                                    <div>TOTAL: {totalPrice} د.إ </div>
                                </div>
                                <div className="total">
                                    <div>Total</div>
                                    <div> د.إ {totalPrice}</div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="check-in-out">
                                <div className="check">
                                    ...
                                    <div onClick={() => setOpenCalendar(true)}>
                                        <div
                                            className="subtitle">{data.value[0] && data.value[0].$D ? data.value[0].$D + ' ' + getMonthName(data.value[0].$M) + ' ' : 'Add dates '}</div>
                                    </div>
                                </div>
                                <div className="check">
                                    <div>Check-out</div>
        ...
        <div className="about-map">
            <div className="info-title">About the neighborhood</div>

            <div className="map-amenities">
                <div className="amenities-collection">
                    {mapAmeneties.map((amenit, index) => (
                        <div key={index}>
                            {amenit.name}
                        </div>
                    ))}
